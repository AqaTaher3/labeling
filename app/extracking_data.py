...
def create_intermediate_frames(frames_list: list):
    ...
    for i in range(len(frames_list) - 1):
        if frames_list[i]['enabled'] is True:
            ...
            for j in range(frame1_value, frame2_value):
                ...
                new_frame = {
                    ...
                    'x': round((frame2['x'] - frame1['x']) * frame_ratio +
                               frame1['x'], 2),
                    'y': round((frame2['y'] - frame1['y']) * frame_ratio +
                               frame1['y'], 2),
                    'width': round((frame2['width'] - frame1['width']) *
                                   frame_ratio + frame1['width'], 2),
                    'height': round((frame2['height'] - frame1['height']) *
                                    frame_ratio + frame1['height'], 2)
                            }
...
def converting_relative_size_to_absolute(video_for_converting,
                                         relative_list: list):
    (original_width, original_height) = calculate_relevant_dimensions
    (video_for_converting)
    list = relative_list
    for rel in list:
